...
\emph{magnitude and ranking} of genuine effects are represented. Two stand out ---
importance grows super-linearly, roughly quadratically, in a feature's linear
effect size (Section~\ref{sec:linear}); and a genuinely non-linear effect and a
linear effect on a skewed feature yield \emph{numerically identical} importances
(Section~\ref{sec:confound}), so the measure cannot separate an effect's shape
from a feature's distribution. The framing question is then practical:
