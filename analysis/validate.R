...
## ---------------------------------------------------------------------------
## Claim 2: pure (parity) interaction. OLS blind (R^2~0), RF captures it,
##          and interaction variables outrank the independent variable c.
## ---------------------------------------------------------------------------
cat("\n=== Claim 2: parity interaction y = c/10 + (a+b) mod 2 ===\n")
set.seed(42); n <- 10000
ib <- function(n) round(runif(n))
df <- tibble(a = ib(n), b = ib(n), c = ib(n), y = c/10 + (a + b) %% 2)
ols <- lm(y ~ a + b + c, df)
cat("  OLS R^2                 :", round(summary(ols)$r.squared, 3), "\n")
cat("  OLS |coef| a,b,c        :", round(abs(coef(ols)[-1]), 3), "\n")
mrf <- ranger(y ~ a + b + c, df, importance = "impurity", num.trees = 500)
cat("  RF  OOB R^2             :", round(mrf$r.squared, 3), "\n")
v <- imp(mrf)
cat("  RF  vip a,b,c           :", round(v, 1), "\n")
cat("  interaction vars > c ?  :", (min(v["a"], v["b"]) > v["c"]), "\n")
